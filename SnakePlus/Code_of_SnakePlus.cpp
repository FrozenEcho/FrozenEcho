...
/*双人竞赛模式中，所有关于蛇的存储副本都要两份，食物会同时出现单份*/
// 全局变量定义
...
int main3() //  主函数
{
	...
	while (1)   // 一直循环
	{
		...
		if (isFailure == 1 || isFailure == 2 || isFailure == 3 || isFailure == 4) //  如果游戏失败 isFailure =1||2||3||4
		{
			...
			outtextxy(100, 180, _T("获胜者为：")); //  输出文字内容
			if (isFailure == 1 || isFailure == 3) {
				outtextxy(400, 180, _T("玩家2！"));
				outtextxy(100, 240, _T("得分为："));
				TCHAR s2[5];
				_stprintf_s(s2, _T("%d"), get_scores2());		// 高版本 VC 推荐使用 _stprintf_s 函数
				outtextxy(400, 240, s2); //  输出scores
			}
			else if (isFailure == 2 || isFailure == 4) {
				outtextxy(400, 180, _T("玩家1！"));
				outtextxy(100, 240, _T("得分为："));
				TCHAR s1[5];
				_stprintf_s(s1, _T("%d"), get_scores1());		// 高版本 VC 推荐使用 _stprintf_s 函数
				outtextxy(400, 240, s1); //  输出scores
			}
